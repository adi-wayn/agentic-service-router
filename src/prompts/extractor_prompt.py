...
2. Extract the `site_location`, `affected_area_or_room`, and `specific_equipment` with high precision.
...
7. Assess safety risks for the `safety_assessment`. Set `has_immediate_hazard` to true if life safety or severe asset damage is active. Set `is_life_safety_affected` as appropriate.
8. Extract any `access_window_or_availability` and `on_site_contact_info`.
9. Document any `detected_conflicts`.
