...
from src.prompts.clarifier_prompt import (
...
def clarifier_node(state: TriageState) -> TriageState:
    ...
    prompt = build_clarifier_user_prompt(
        raw_text, missing_fields, conflicts, clarification_state.clarification_history
    )
    ...
    try:
        ...
        state["audit_trace"].append(
            f"Clarification generated {len(q_texts)} question(s)."
        )
...
def feedback_node(state: TriageState) -> TriageState:
    ...
    for q in clarification_state.clarification_questions:
        ...
        clarification_state.clarification_history.append(
            {"question": q.question_text, "answer": simulated_answer}
        )
    ...
    state["audit_trace"].append(
        f"Feedback ingested for {answered} question(s). Loop count now {clarification_state.loop_count}."
    )
