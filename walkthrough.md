...
1. **Directories Created**:
   ...
   *   `src/prompts/` - Dedicated location for `extract_prompt.txt`, `clarify_prompt.txt`, etc., as per the SDD specs.
   *   `eval/` - For the 11-case benchmark evaluation suite.
   *   `data/` - For static data files.

2. **Data Relocation**:
   *   Moved `service_catalogue.json` and `test_requests.json` into the newly created `data/` directory to declutter the root.
...
4. **Configuration & Secrets**:
   ...
   *   Created `src/config.py` using `python-dotenv` to safely load variables into a central `Config` class, guaranteeing that all downstream nodes can access the active LLM provider choice or catalogue data path securely.

## Next Steps
The baseline configuration is successfully in place. We are now ready to move onto **Task 1.2: Implement Pydantic Data Models**.
