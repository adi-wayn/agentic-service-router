...
# Ensure project root is in sys.path when executed directly as `python eval/run_evaluation.py`
...
from eval.ground_truth import load_ground_truth  # noqa: E402
...
def run_benchmark(verbose: bool = False):
    ...
    raw_requests = load_requests(
        ["data/test_requests.json", "data/custom-hard-requests.json"]
    )
    ...
    print(
        f"Loaded {len(raw_requests)} requests. Initializing TriageAgent (LLM Provider: {os.environ.get('LLM_PROVIDER', 'gemini')})..."
    )
    agent = TriageAgent()
    ...
    print(
        "Note: Applying 20s pacing between requests to respect free-tier provider RPM limits."
    )
    ...
    for i, req in enumerate(raw_requests):
        ...
        try:
            ...
            if decision is None:
                print(
                    f"Error: decision is None! Final state keys: {final_state.keys()}"
                )
        ...
        pred_action = (
            decision.initial_routing_action
            if decision.initial_routing_action
            else "UNKNOWN"
        )
        pred_urgency = (
            decision.extracted_entities.assessed_real_urgency
            if decision.extracted_entities
            else "UNKNOWN"
        )
        pred_temp = (
            decision.match_result.top_template_id if decision.match_result else None
        )
        pred_missing = (
            decision.initial_missing_fields
            if decision.initial_missing_fields is not None
            else []
        )
        pred_conf = (
            decision.routing_result.confidence_score if decision.routing_result else 0.0
        )

        clarif_qs = (
            decision.clarification_state.clarification_questions
            if decision.clarification_state
            else []
        )
        q_targets = [q.target_field for q in clarif_qs]
        l_count = (
            decision.clarification_state.loop_count
            if decision.clarification_state
            else 0
        )
        ...
        if verbose:
            print(
                f"  -> GT Action: {gt.expected_routing_action} | Pred Action: {pred_action}"
            )
            print(
                f"  -> GT Urgency: {gt.expected_real_urgency} | Pred Urgency: {pred_urgency}"
            )
            print(
                f"  -> Rationale: {decision.finalizer_synthesis.decision_rationale if decision.finalizer_synthesis else 'None'}\n"
            )

        # Provider rate limiting pacing (protects against RPM exhaustion on free tier)
        time.sleep(20)
    ...
    jaccards = [
        compute_jaccard_iou(t, p)
        for t, p in zip(y_true_missing_list, y_pred_missing_list)
    ]
    ...
    clarif = compute_clarification_metrics(
        loop_counts, y_true_missing_list, generated_questions_list
    )
    ...
    print(
        f"   - CONFIDENT Precision: {conf_matrix['CONFIDENT_RECOMMENDATION']['precision']:.3f}, Recall: {conf_matrix['CONFIDENT_RECOMMENDATION']['recall']:.3f}"
    )
    print(
        f"   - CLARIFY Precision:   {conf_matrix['NEEDS_CLARIFICATION']['precision']:.3f}, Recall: {conf_matrix['NEEDS_CLARIFICATION']['recall']:.3f}"
    )
    print(
        f"   - HUMAN Precision:     {conf_matrix['ROUTE_TO_HUMAN']['precision']:.3f}, Recall: {conf_matrix['ROUTE_TO_HUMAN']['recall']:.3f}"
    )

    print("\n2. Safety Critical Metrics:")
    print(
        f"   - P1 Safety Recall:    {safety_metrics['p1_recall'] * 100:.1f}% (Target: 100.0%)"
    )
    print(
        f"   - P1 False Neg Rate:   {safety_metrics['p1_fnr'] * 100:.1f}% (Target: 0.0%)"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Print step-by-step predictions"
    )
    args = parser.parse_args()

    if not os.environ.get("GEMINI_API_KEY"):
        print(
            "CRITICAL WARNING: GEMINI_API_KEY environment variable is not set. Execution will likely fail."
        )
