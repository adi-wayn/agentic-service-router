...
def compute_confusion_matrix(y_true: List[str], y_pred: List[str]) -> Dict[str, Any]:
    ...
    weighted_f1 = sum(metrics[c]["f1"] * ((tp[c] + fn[c]) / total_true) for c in classes) if total_true > 0 else 0.0
    metrics["weighted_f1"] = weighted_f1
    
    return metrics

def compute_safety_metrics(y_true_urgency: List[str], y_pred_urgency: List[str]) -> Dict[str, Any]:
    tp_p1 = 0
    fn_p1 = 0
    fp_p1 = 0
    tn_p1 = 0
...
def compute_jaccard_iou(expected_missing: List[str], actual_missing: List[str]) -> float:
...
def compute_calibration(y_true_binary: List[int], y_prob: List[float], n_bins: int = 10) -> Dict[str, float]:
    if not y_prob:
...
def compute_template_accuracy(y_true_temp: List[str], y_pred_temp: List[str]) -> float:
    if not y_true_temp: return 0.0
    correct = sum(1 for t, p in zip(y_true_temp, y_pred_temp) if t == p)
    return correct / len(y_true_temp)

def compute_clarification_metrics(loop_counts: List[int], expected_missing_list: List[List[str]], generated_questions_list: List[List[str]]) -> Dict[str, Any]:
    """
    Computes CR, MTTR, Question Specificity, and Redundancy Index for cases requiring clarification.
    """
    if not loop_counts:
        return {"convergence_rate": 0.0, "mttr": 0.0, "specificity": 0.0, "redundancy": 0.0}
    ...
    for expected_missing, generated_questions in zip(expected_missing_list, generated_questions_list):
    ...
    redundancy_index = redundant_questions / total_questions if total_questions > 0 else 0.0
